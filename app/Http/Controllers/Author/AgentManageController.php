...
use App\Parcel;
use App\Disclamer; // Added import
...
class AgentManageController extends Controller
{
    ...
    public function notice() {
        $notice = Disclamer::find(2); // Assuming ID 2 for agent notice, adjust if necessary
    ...
    public function noticestore(Request $request) {
        Disclamer::updateOrCreate(
            [
                'id' => 2, // Assuming ID 2 for agent notice, adjust if necessary
            ],
            [
                'title' => $request->title,
                'type' => 'agent', // Added type for agent notice
            ]
        );
        Toastr::success('message', 'Notice updated successfully!');
        return back();
    }
