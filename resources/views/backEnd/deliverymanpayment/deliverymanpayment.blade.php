                    ...
                    <div class="col-sm-12 col-md-12 col-lg-12">
                            ...
                            <div class="col-sm-12">
                                <div class="manage-button">
                                    <div class="body-title">
                                        <h5>Deliveryman payment history</h5>
                                    </div>
                                </div>
                            </div>
                            <h4 class="alart"><img class="icon" src="{{asset('warning.png')}}" alt="">Before processing the deliveryman payment, ensure that you export the CSV file</h4>
                        ...
                        <div class="row">
                          ...
                          <!-- col end -->
                            ...
                            <div class="card-body">
                                <form action="{{ url('editor/deliveryman/commission-confirm-payment') }}" method="POST" id="myform"
                                    ...
                                    <button type="button" class="bulkbutton bulk-status-btn" id="MakepayButton">Make Deliveryman Payment</button>
                                    <a href="javascript:void(0)"
                                        data-href="{{ route('editor.deliveryman.payment.export-csv') }}"
                                    ...
                                    <table id="" class="table table-bordered table-striped custom-table">
                                        ...
                                        <tbody>
                                            @foreach ($Deliverymans as $key => $value)
                                            @if ($value->charge > 0) 
                                                    <tr>
                                                        <td><input type="checkbox" value="{{ $value->id }}"
                                                                name="deliveryman_id[]" form="myform"
                ...
                <div class="modal-content">
                    <div class="modal-body">
                        <div class="container-fluid text-center">
                           ...
                           <h6> Before processing the deliveryman payment, ensure that you export the CSV file.</h6>
                        </div>
                    </div>
    ...
    <script>
        function exportMerchantPayment(button, type) {
            ...
            if (searchIDs.length < 1) {
                toastr.warning('Please select at-least one deliveryman', 'Oops!');
                return false;
            }
            var href = $(button).attr('data-href');
            var searchIDsString = encodeURIComponent(JSON.stringify(searchIDs));
            var full_url = href + '?deliverymans=' + searchIDsString + '&type=' + type;
        ...
        function validateSubmitbutton() {
            ...
            if (searchIDs.length < 1) {
                ...
                // var url = "{{ route('editor.deliveryman.payment.export-csv') }}";
        ...
        function exportCSV(url, searchIDs) {
            var searchIDsString = encodeURIComponent(JSON.stringify(searchIDs));
            var full_url = url + '?deliverymans=' + searchIDsString + '&type=' + 'csv';
            window.open(full_url, '_blank').focus();
        }
