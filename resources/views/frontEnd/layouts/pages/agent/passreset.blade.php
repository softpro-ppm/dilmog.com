...
@section('title','Agent Login | Dilmog Logistics')
@section('content')
<style>
html, body {
    overflow-x: hidden;
...
.reset-card input[type="email"].is-invalid {
    border-color: #e50914;
}
    ...
    <div class="merchant-logo">
        <a href="{{ url('/') }}">
            @if(isset($darklogo) && count($darklogo))
                <img src="{{ asset($darklogo[0]->image) }}" alt="Agent Logo" style="max-height:48px; width:auto; display:block;">
            @else
                <img src="{{ asset('assets/img/logo.png') }}" alt="Agent Logo" style="max-height:48px; width:auto; display:block;">
            @endif
        </a>
    </div>
</div>
<div class="center-flex">
    <div class="reset-card">
        <h1><strong>Agent Password Forget</strong></h1>
        ...
        <form action="{{url('auth/agent/password/reset')}}" method="post">
            @csrf
        ...
        <div style="text-align:center; margin-top: 18px; font-size: 1.1rem; color: #222;">
            <span>Back to </span><a href="{{ url('agent/login') }}" style="text-decoration: underline; color: #222; font-weight: 500;">Sign In</a>
        </div>
    </div>
</div>
@endsection