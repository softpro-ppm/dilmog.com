...
@section('content')
<script>
    var showPrintButtons = @json(in_array(request()->segment(4), ['pending', 'picked-up']));
</script>
...
<script>
    ...
    $(document).ready(function(event) {
        ...
        var table33 = $('#example333').DataTable({
            ...
            ajax: {
                url: "{{ request()->has('month') && request()->month == 1 ? url('merchant/get_parcel_data_monthTest') : url('merchant/get_parcel_dataTest') }}" + '/' + slug,
                data: {
